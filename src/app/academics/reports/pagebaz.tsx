...
import { generatePOSReceipt, generateInvoicePDF, ReceiptData } from '@/utils/receipts';
import { formatToWords } from '@/utils/numbers';
...
const ReportsPage = () => {
  ...
  const schoolInfo = {
    name: 'IBUN BAZ PRIMARY SCHOOL',
    address: 'BUGABWE, IGANGA',
    po_box: 'P.O. BOX 662, IGANGA MUNICIPALITY',
    logo_url: '/schoollogo.png',
    contact: 'Tel: 0704-550-460 / 0702-387-803',
    center_no: 'UNEB CENTRE No: 080461',
    registration_no: 'Reg no: PPS/N/297',
    arabic_name: 'مدرسة ابن باز الابتدائية',
    arabic_address: 'بوغابوي، إيغانغا',
    arabic_contact: 'هاتف:0704-550-460 / 0702-387-803',
  ...
  const submitPayment = async () => {
    ...
    try {
      // ...existing payment submission code...
      ...
      if (d.success) {
        ...
        const receiptData: ReceiptData = {
          schoolInfo: {
            name: 'IBUN BAZ PRIMARY SCHOOL',
            address: 'BUGABWE, IGANGA',
            contact: 'Tel: 0702-667-010',
            logo: '/schoollogo.png'
          },
          payment: {
            receiptNo: d.receipt_no || `RCP${Date.now()}`,
            invoiceNo: d.invoice_no || `INV${Date.now()}`,
            date: new Date().toLocaleString(),
            amount: parseFloat(payForm.amount),
            amountWords: formatToWords(parseFloat(payForm.amount)),
            method: payForm.method,
            paidBy: payForm.paid_by,
            reference: payForm.reference
          },
          student: {
            id: payStudent.id.toString(),
            name: payStudent.name,
            class: payStudent.class_name || ''
          },
          item: {
            name: payItem.item,
            amount: payItem.outstanding,
            balance: payItem.newBalance
          }
        };

        // Generate POS receipt
        const receiptHtml = generatePOSReceipt(receiptData);
        ...
        // Generate & open invoice PDF
        const invoicePDF = generateInvoicePDF(receiptData);
      ...
      setPayStatus({loading: false, error: '', success: 'Payment recorded & documents generated'});
      await loadFeeItems();
    } catch (error) {
                ...
                <div key={student.student_id} style={styles.reportPage}>
                  ...
                  <div style={styles.header}>
                    ...
                    <div className="text-right font-bold text-xl rtl:text-right ltr:text-left" style={{ direction: 'rtl', textAlign: 'right', flex: 1 }}>
                      ...
                      <p>{schoolInfo.arabic_contact}</p>
                      <p>{schoolInfo.arabic_center_no}</p>
                      <p>{schoolInfo.arabic_registration_no}</p>
                    </div>
                  </div>
                  {/* Banner */}
                  ...
                  <div style={styles.studentDetails}>
                    ...
                    {/* Use photo_url from API, fallback to placeholder */}
                    <Image
                      ...
                      // unoptimized={student.photo.startsWith('http')} // Skip optimization for remote images
                      // onError={(e) => {
                      //   const target = e.target as HTMLImageElement;
                      //   if (!target.src.includes('schoollogo.png')) {
                      //     target.src = '/schoollogo.png';
                      //   }
                      // }}
                    />
                    <div>
