...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const theologyClassId = searchParams.get('theology_class_id');

    connection = await getConnection();

    // Enhanced query to include theology class information
    let sql = `
      SELECT DISTINCT
        s.id,
        s.person_id,
    ...
    // Add theology class filter
    if (theologyClassId) {
      sql += ` AND tc.id = ?`;
      params.push(theologyClassId);
    }
