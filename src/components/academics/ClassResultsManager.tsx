...
export default function ClassResultsManager() {
  ...
  const handleOpenModal = () => {
    setOpen(true);
    setKlass(classes.find(c => c.id == filters.class_id) || null);
    setSubject(subjects.find(s => s.id == filters.subject_id) || null);
    setRtype(types.find(rt => rt.id == filters.result_type_id) || null);
    setTerm(terms.find(t => t.id == filters.term_id) || null);
    setRows([]);
  };
  ...
  const fetchMissingRows = async (bulkFilters: any) => {
    ...
    try {
      ...
      else setRows((data.data || []).map((r: any) => ({ ...r, score: null, grade: null, remarks: null })));
    } catch (e: any) {
  ...
  // Export to PDF
  const exportToPDF = async (scope: 'learner' | 'class' | 'school', filters: any) => {
    ...
    // Table: Add rows
    const rows = filteredMarklist.map(row => [
      row.name,
      row.class_name,
      ...subjectColumns.map(s => row.scores[s.id] ? row.scores[s.id].score : '-'),
      row.total ?? '-',
      row.min ?? '-',
      row.max ?? '-',
  ...
  const exportToExcel = (scope: 'learner' | 'class' | 'school', data: any[]) => {
    ...
    const columnWidths = [
      ...
      { wpx: 100 }, // Class
      { wpx: 80 },  // Subjects (auto-adjusted later)
      { wpx: 80 },
      { wpx: 80 },
      { wpx: 80 },
      { wpx: 80 },
    ...
    XLSX.utils.sheet_set_col_widths(ws, columnWidths);
    
    // Auto-adjust width for subject columns
    subjectColumns.forEach((_, i) => {
      const colIndex = i + 2; // Offset by 2 for first two columns (Name, Class)
      const maxWidth = Math.max(...data.map(row => {
        const cellValue = row.scores ? row.scores[i] : '';
        return typeof cellValue === 'string' ? cellValue.length * 7 : 0;
      }));
      ws['!cols'][colIndex] = { wpx: Math.min(Math.max(maxWidth, 80), 150) };
    });
    ...
    <div className="mt-8 space-y-10">
      ...
      <div className="flex flex-wrap gap-2 items-center mb-4">
        ...
        <button
          ...
          className="ml-auto px-4 py-2 rounded-lg text-xs font-semibold bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-pink-600 text-white"
        >
          {t('add_edit_results')}
        </button>
      </div>
      ...
      <div className="flex gap-4">
        <button
          onClick={() => exportToPDF('learner', filters)}
        ...
        <button
          onClick={() => exportToPDF('class', filters)}
          className="px-4 py-2 rounded-lg text-xs font-semibold bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-pink-600 text-white"
        >
          Export Class PDF
        </button>
        <button
          onClick={() => exportToPDF('school', filters)}
          className="px-4 py-2 rounded-lg text-xs font-semibold bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-pink-600 text-white"
        >
          Export School PDF
        </button>
        <button
          ...
          className="px-4 py-2 rounded-lg text-xs font-semibold bg-gradient-to-r from-green-600 via-teal-600 to-blue-600 text-white"
        >
          Export Learner Excel
        </button>
        <button
          onClick={() => exportToExcel('class', filteredMarklist)}
          className="px-4 py-2 rounded-lg text-xs font-semibold bg-gradient-to-r from-green-600 via-teal-600 to-blue-600 text-white"
        >
          Export Class Excel
        </button>
        <button
          onClick={() => exportToExcel('school', filteredMarklist)}
          className="px-4 py-2 rounded-lg text-xs font-semibold bg-gradient-to-r from_green-600 via-teal-600 to-blue-600 text-white"
        >
          Export School Excel
        </button>
      </div>
