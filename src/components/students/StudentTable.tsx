...
export const StudentTable: React.FC = () => {
  ...
  const [bulkTheologyClass, setBulkTheologyClass] = useState('');
  ...
  const { data, isLoading, mutate } = useSWR(
    `${API_BASE}/students/full?q=${encodeURIComponent(query)}${selectedClass ? `&class_id=${selectedClass}` : ''}${selectedStream ? `&stream_id=${selectedStream}` : ''}${selectedGender ? `&gender=${selectedGender}` : ''}${selectedStatus ? `&status=${selectedStatus}` : ''}${selectedTheologyClass ? `&theology_class_id=${selectedTheologyClass}` : ''}`,
    fetcher
  );
  ...
  // New theology class bulk assignment handler
  const handleBulkTheologyAssign = async () => {
    if (selectedLearners.length === 0) {
      Swal.fire('Error', 'Please select at least one student.', 'error');
      return;
    }

    if (!bulkTheologyClass) {
      Swal.fire('Error', 'Please select a theology class to assign.', 'error');
      return;
    }

    const selectedTheologyClassName = classOptions.find((cls: any) => cls.id === parseInt(bulkTheologyClass))?.name || 'Unknown Theology Class';

    const confirm = await Swal.fire({
      title: 'Bulk Theology Class Assignment',
      html: `
        <div class="text-center">
          <p>Assign <strong>${selectedLearners.length}</strong> selected students to:</p>
          <div class="mt-3 p-3 rounded-lg bg-green-50">
            <span class="font-semibold text-green-800">${selectedTheologyClassName}</span>
          </div>
          <p class="mt-3 text-sm text-gray-600">This will update their theology class enrollment.</p>
        </div>
      `,
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Yes, Assign Theology Class',
      cancelButtonText: 'Cancel',
      customClass: {
        popup: 'rounded-2xl',
    ...
    try {
      Swal.fire({
        title: 'Processing...',
        html: 'Assigning theology class to selected students...',
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        },
      ...
      const response = await fetch(`${API_BASE}/students/theology-assign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          student_ids: selectedLearners, 
          theology_class_id: bulkTheologyClass 
        }),
      });
      ...
      if (result.success) {
        Swal.fire({
          title: 'Success!',
          text: `${selectedLearners.length} students have been assigned to ${selectedTheologyClassName}`,
          icon: 'success',
          timer: 3000,
        });
        setSelectedLearners([]);
        setBulkTheologyClass('');
        setShowBulkActions(false);
        mutate();
      } else {
        Swal.fire('Error', result.error || 'Failed to assign theology class.', 'error');
      }
    } catch (error) {
      Swal.fire('Error', 'An unexpected error occurred during bulk theology assignment.', 'error');
    }
  };
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Assign selected students to a class, theology class, or perform other bulk operations.
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      className="input-field w-40 shadow-sm focus:shadow-md transition-shadow duration-200"
                    >
                  ...
                  {/* Theology Class Assignment */}
                  <div className="flex items-center gap-2">
                    <select
                      value={bulkTheologyClass}
                      onChange={(e) => setBulkTheologyClass(e.target.value)}
                      className="input-field w-48 shadow-sm focus:shadow-md transition-shadow duration-200"
                    >
                      <option value="">Select Theology Class...</option>
                      {classOptions.map((cls: any) => (
                        <option key={`theology-class-${cls.id}`} value={cls.id}>
                          {cls.name}
                        </option>
                      ))}
                    </select>
                    ...
                    <button
                      onClick={handleBulkTheologyAssign}
                      disabled={!bulkTheologyClass}
                      className="btn-secondary bg-gradient-to-r from-green-500 to-emerald-600 text-white gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transition-all duration-200"
                    >
                      <Users className="w-4 h-4" />
                      Assign Theology
                    </button>
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div
              ...
              className="overflow-hidden"
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                ...
                <select
                  ...
                  onChange={(e) => {
                    setSelectedTheologyClass(e.target.value);
                    setPage(1);
                  }}
                  className="input-field shadow-sm focus:shadow-md transition-all duration-200"
                >
                  <option value="">All Theology Classes</option>
                  {classOptions.map((cls: any) => (
                    <option key={`theology-class-${cls.id}`} value={cls.id}>
                      {cls.name}
                    </option>
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <div className="text-sm text-gray-900 dark:text-gray-100">
                        <span className="font-medium">Main:</span> {student.class_name || 'No Class'}
                      </div>
                      <div className="text-sm text-gray-900 dark:text-gray-100">
                        <span className="font-medium text-green-600">Theology:</span> {student.theology_class_name || 'Not Assigned'}
                      </div>
                      {student.stream_name && (
          ...
          {paginatedRows.map((student) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="font-medium text-gray-600 dark:text-gray-400">Main Class:</span>
                  <div className="text-gray-900 dark:text-gray-100">
                    {student.class_name || 'No Class'}
                  </div>
                </div>
                <div>
                  <span className="font-medium text-green-600 dark:text-green-400">Theology Class:</span>
                  <div className="text-gray-900 dark:text-gray-100">
                    {student.theology_class_name || 'Not Assigned'}
                  </div>
                </div>
                <div>
